(function(global) {
  ...
  var ParseHTML = function(appConf) {
    var conf = this.appConf = appConf || new Config();
    this.filterArr = [];
    this.removeArr = conf.get("remove");
    this.excludeArr = conf.get("exclude");
    var minCSSExt = conf.get("min-css-ext");
    if (minCSSExt) {
      this.excludeArr.push(minCSSExt.replace(/\./g, '\.') + '$');
    }
    var minJSExt = conf.get("min-js-ext");
    if (minJSExt) {
      this.excludeArr.push(minJSExt.replace(/\./g, '\.') + '$');
    }
  };
    ...
    fileArr.forEach(delegate(this, function(filePath) {
      if (fs.existsSync(filePath)) {
        try {
          ...
          var removeArr = this.removeArr;
          var removeElement = function(el) {
            var remove = false;
            if (el && el.attribs) {
              var resourcePath = el.attribs.href || el.attribs.src;
              if (resourcePath && removeArr && removeArr.length) {
                removeArr.forEach(function(removeRE, idx) {
                  var re = new RegExp(removeRE, 'gi');
          ...
          var excludeElement = function(el) {
            var exclude = false;
            if (el && el.attribs) {
              var resourcePath = el.attribs.href || el.attribs.src;
              if (resourcePath && excludeArr && excludeArr.length) {
                excludeArr.forEach(function(excludeRE, idx) {
                  var re = new RegExp(excludeRE, 'gi');
                  if (resourcePath.match(re)) {
                    exclude = true;
                  }
                });
...
//              };
          ...
          var handler = new Handler(delegate(this, function(err, dom) {
            ...
            if (el.name === 'link' || el.name === 'script') {
              ...
              if (removeElement(el)) {
                DomUtils.removeElement(el);
              } else if (excludeElement(el)) {
              } else {
                var resourcePath = el.attribs.href || el.attribs.src;
                if (resourcePath) {
                  addChild(fs.getExtension(resourcePath), el, function(err) {
                    if (err) {
                      callback(err);
                    }
                  });
