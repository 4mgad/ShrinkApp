(function(global) {
  ...
  ParseHTML.prototype.applyFilter = function(filePath, callback) {
    ...
    var check = Utils.delegate(this, function(decrement) {
      if (decrement) {
        count--;
      }
      if (count <= 0) {
                            ...
                            for (var i = 0; i < _elArr.length; i++) {
                              ...
                              if (resultArr[i]) {
                                if (el.attribs.src) {
                                  ...
                                  addChild(Utils.getExtension(el.attribs.src), el, function(err) {
                                    if (err) {
                                      callback(err);
                                    }
                                  });
                                ...
                                if (el.attribs.href) {
                                  ...
                                  addChild(Utils.getExtension(el.attribs.href), el, function(err) {
                                    if (err) {
                                      callback(err);
                                    }
                                  });
